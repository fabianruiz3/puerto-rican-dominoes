...
async def run_arena_endpoint(bot_a: UploadFile=File(...), bot_b: UploadFile=File(...), num_matches: int=Form(default=1000), target_points: int=Form(default=200)):
    ...
    try:
        results = run_arena(bot_a=bot_a_inst, bot_b=bot_b_inst, num_matches=num_matches, target_points=target_points)
    except Exception as e:
